...
export default function RoutePreviewMap({ idPrefix, path, onOpen, style }: Props) {
  ...
  return (
    <View style={style}>
      ...
      {onOpen && path.length > 0 && (
        <MapPreviewOverlay label={t("map.showMapView")} onPress={onOpen} startPosition={path[0]} />
      )}
    </View>
  );
}
