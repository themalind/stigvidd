...
It is generated from `web/openapi.json`, which is **not committed**: the StigviddAPI build
exports it, so `cd backend && dotnet build` is what produces it.

Both steps run for you on an ordinary build — the dev server and the production build in `web/`
refresh the document and then the client — so in practice you build, then commit whatever
appeared under `src/api/generated`. To do it explicitly:
...
spec, so no backend needs to be running — but a checkout that has never built the backend has no
spec to read. Set `ORVAL_API_URL` to `http://localhost:5265/swagger/v1/swagger.json` to read from
a live API instead.
